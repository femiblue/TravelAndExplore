...
//
// Table class for replies
//
class creplies extends cTable {
	var $ReplyTrigger;
	var $ReplyResponse;
	var $ReplyType;
	var $ReplyValidationResponse;
	var $ReplyStage;
	var $ReplyID;
	...
	function __construct() {
		...
		// ReplyTrigger
		$this->ReplyTrigger = new cField('replies', 'replies', 'x_ReplyTrigger', 'ReplyTrigger', '`ReplyTrigger`', '`ReplyTrigger`', 201, -1, FALSE, '`ReplyTrigger`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ReplyTrigger'] = &$this->ReplyTrigger;

		// ReplyResponse
		$this->ReplyResponse = new cField('replies', 'replies', 'x_ReplyResponse', 'ReplyResponse', '`ReplyResponse`', '`ReplyResponse`', 201, -1, FALSE, '`ReplyResponse`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ReplyResponse'] = &$this->ReplyResponse;

		// ReplyType
		$this->ReplyType = new cField('replies', 'replies', 'x_ReplyType', 'ReplyType', '`ReplyType`', '`ReplyType`', 200, -1, FALSE, '`ReplyType`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ReplyType'] = &$this->ReplyType;

		// ReplyValidationResponse
		$this->ReplyValidationResponse = new cField('replies', 'replies', 'x_ReplyValidationResponse', 'ReplyValidationResponse', '`ReplyValidationResponse`', '`ReplyValidationResponse`', 201, -1, FALSE, '`ReplyValidationResponse`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['ReplyValidationResponse'] = &$this->ReplyValidationResponse;

		// ReplyStage
		$this->ReplyStage = new cField('replies', 'replies', 'x_ReplyStage', 'ReplyStage', '`ReplyStage`', '`ReplyStage`', 3, -1, FALSE, '`ReplyStage`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->ReplyStage->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['ReplyStage'] = &$this->ReplyStage;

		// ReplyID
		$this->ReplyID = new cField('replies', 'replies', 'x_ReplyID', 'ReplyID', '`ReplyID`', '`ReplyID`', 19, -1, FALSE, '`ReplyID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->ReplyID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['ReplyID'] = &$this->ReplyID;
	}
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('ReplyID', $rs))
				ew_AddFilter($where, ew_QuotedName('ReplyID') . '=' . ew_QuotedValue($rs['ReplyID'], $this->ReplyID->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->ReplyID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@ReplyID@", ew_AdjustSql($this->ReplyID->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("repliesview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("repliesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "repliesadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("repliesedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("repliesadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("repliesdelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->ReplyID->CurrentValue)) {
			$sUrl .= "ReplyID=" . urlencode($this->ReplyID->CurrentValue);
		} else {
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				if ($this->ReplyTrigger->Exportable) $Doc->ExportCaption($this->ReplyTrigger);
				if ($this->ReplyResponse->Exportable) $Doc->ExportCaption($this->ReplyResponse);
				if ($this->ReplyType->Exportable) $Doc->ExportCaption($this->ReplyType);
				if ($this->ReplyValidationResponse->Exportable) $Doc->ExportCaption($this->ReplyValidationResponse);
				if ($this->ReplyStage->Exportable) $Doc->ExportCaption($this->ReplyStage);
				if ($this->ReplyID->Exportable) $Doc->ExportCaption($this->ReplyID);
			} else {
				if ($this->ReplyType->Exportable) $Doc->ExportCaption($this->ReplyType);
				if ($this->ReplyStage->Exportable) $Doc->ExportCaption($this->ReplyStage);
				if ($this->ReplyID->Exportable) $Doc->ExportCaption($this->ReplyID);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					if ($this->ReplyTrigger->Exportable) $Doc->ExportField($this->ReplyTrigger);
					if ($this->ReplyResponse->Exportable) $Doc->ExportField($this->ReplyResponse);
					if ($this->ReplyType->Exportable) $Doc->ExportField($this->ReplyType);
					if ($this->ReplyValidationResponse->Exportable) $Doc->ExportField($this->ReplyValidationResponse);
					if ($this->ReplyStage->Exportable) $Doc->ExportField($this->ReplyStage);
					if ($this->ReplyID->Exportable) $Doc->ExportField($this->ReplyID);
				} else {
					if ($this->ReplyType->Exportable) $Doc->ExportField($this->ReplyType);
					if ($this->ReplyStage->Exportable) $Doc->ExportField($this->ReplyStage);
					if ($this->ReplyID->Exportable) $Doc->ExportField($this->ReplyID);
